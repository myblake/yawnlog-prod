...
jQuery.fn.selectToUISlider = function(settings){
	...
	//style the dt's (change math to scale dts so they are split at midnight)
	sliderComponent.find('.ui-slider-scale dt').each(function(i){
		jQuery(this).css({
			'left': ((100 /( groups.length))*i).toFixed(2) + '%'
		});
		// this adds a class to each for future updates from the datepicker
		jQuery(this).addClass("date_"+i);
	});
